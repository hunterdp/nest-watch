...
class Nest:
    ...
    def __init__(self, username, password, serial=None, index=0, units="F"):
        ...
        LOG.debug("Created a Nest Instance with username: %s, serial: %s, index: %s, units: %s.",
    ...
    def login_developer_account(self, client_id, client_secret, client_pin):
        ...
        # except Exception as e:
            ...
            # LOG.critical("Unable to get access token.")
            # LOG.critical("Error raised: %s", e)
            # raise e
...
def _configure_logger(name):
    ...
    fmt_str = fmt_str_1 + "%(funcName)-12s %(processName)s:%(process)d %(message)-s"
...
if __name__ == "__main__":
    ''' Initializes the logger and verify prerequisites '''
